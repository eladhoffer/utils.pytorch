...
def filtered_parameter_info(model, module_type_fn=None, module_name_fn=None, parameter_name_fn=None, memo=None):
    if memo is None:
        memo = set()

    for module_name, module in model.named_modules():
        if module_type_fn is not None and not module_type_fn(module):
            continue
...
class FilterParameters(object):
    def __init__(self, source, module_type=None, module_name=None, parameter_name=None):
    ...
    def filter(self, module_type=None, module_name=None, parameter_name=None):
        for p_info in self._filtered_param_info:
            if (module_type is None or module_type(p_info['named_module'][1])
...
if __name__ == '__main__':
    ...
    filterd_params = FilterParameters(model,
                                      module_type=lambda m: isinstance(
                                          m, torch.nn.Linear),
